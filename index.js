...
function move(gameState) {
 ...
 // const food = gameState.board.food;
  ...
  for (const move in moves) {
    ...
    //prevent collision
    const newPos = moves[move];
    //if the new position is out of bounds then the move is not safe
    if (
      newPos.x < 0 || newPos.x >= boardWidth ||
      newPos.y < 0 || newPos.y >= boardHeight
    ) {
      isMoveSafe[move] = false;
    }
    
    if (!isMoveSafe[move]) continue;  // skip unsafe moves

    // use foodFill to score this move
    const spaceScore = floodFill(newPos, boardWidth, boardHeight, myBody, opponents);
...
//The idea is to find open space vs occupied space with 4 way fill
function floodFill(start, boardWidth, boardHeight, myBody, opponents){
   ...
   return visited.length; // total reachable open space
